...
function writeNext(noTeam)

{
  var stream = fs.createWriteStream("./test_output/noTeam.csv");
  ...
  stream.on('finish', function() {
      console.log("Saved student without team.");
      console.log("Done");
      process.exit();
  });
}

function WriteOuptput() {
  var out = generateOutput()
  var output = out[0]; var noTeam= [1]
  var stream = fs.createWriteStream("./test_output/teamScores.csv");
  //exits when the process of writing is done
  stream.on('finish', function() {
      console.log("Saved student max score.");
      writeNext(noTeam)
  });
  /*
